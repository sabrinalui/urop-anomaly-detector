...
insert_row_sql = """ INSERT INTO tweets(loc,body,time_stamp)
              VALUES(?,?,?) """
...
class StreamingAnomalyHandler(tweepy.StreamListener):
	...
	def sanitize(self, status):
		...
		loc = self.get_place(status)
		return ((body,timestamp),loc)

	def get_place(self,tweet):
...
class UpdaterThread(Thread):
	...
	def update_models(self):
		...
		for tweet in tweets:
			queue[tweet[0]].append((tweet[1],tweet[2]))
